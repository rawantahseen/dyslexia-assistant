...
// ── Intersection Observer (cards & contact) ──
...
async function analyzeText(text)    { return await callAPI("/analyze", text); }
...
async function processUserView(text) {
  const response = await fetch(`${API_BASE_URL}/process?user_view=true`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ text })
  });
...
async function sendTextToAI(actionType) {
  ...
  try {
    ...
    switch (actionType) {
      case "analyze": {
        const [analyzeData, processData] = await Promise.all([
          analyzeText(text),
          processUserView(text)
        ]);
        data = {
          ...analyzeData,
          improvement_summary: processData.improvement_summary,
          words_to_watch: processData.words_to_watch
        };
        break;
      }
      case "simplify":
...
function showResult(data, actionType) {
  ...
  switch (actionType) {
    ...
    case "analyze":
      ...
      resultHTML = `
        <div class="mb-4">
          ...
          <div class="mb-3 mt-3 p-3 rounded" style="background: var(--input-bg-color); border-left: 4px solid var(--accent-color)">
            <p class="mb-0"> ${data.improvement_summary || 'No summary available'}</p>
          </div>
        </div>
        <div class="mb-3">
    ...
    case "process":
      ...
      let highlightedSimplified = data.simplified;
      if (data.diff.survived && data.diff.survived.length > 0) {
        data.diff.survived.forEach(item => {
          const wordText = item.word;
          const regex = new RegExp(`\\b${wordText}\\b`, 'gi');
          highlightedSimplified = highlightedSimplified.replace(
            regex,
            `<span class="hard-word-survived" title="Still difficult (score: ${item.simplified_score})">${wordText}</span>`
          );
        });
      }
      ...
      resultHTML = `
        ...
        <div class="mb-5 p-4 rounded shadow-inner" style="background: var(--input-bg-color); border-left: 5px solid var(--accent-color)">
          ...
          <p class="mb-0" style="line-height: 2; font-size: 1rem;">${highlightedSimplified}</p>
        </div>
      `;
      break;
  }
...
document.getElementById("volumeCheckbox").addEventListener("change", function () {
  const resultContent = document.getElementById("resultContent");
  if (this.checked) {
    const utterance = new SpeechSynthesisUtterance(resultContent.innerText);
    utterance.rate = 0.85;
